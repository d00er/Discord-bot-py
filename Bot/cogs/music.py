...
class music_cog(commands.Cog):
    ...
    async def play_next(self, ctx):
        try:
            if self.music_queue:
                ...
                await ctx.send(f"Ahora tocando **{title}**") # Now playing **{title}**
            elif not ctx.voice_client.is_playing():
                await ctx.send("La cola esta vacia!") #queue is empty!
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            if self.vc == None or not self.vc.is_connected():
                ...
                if self.vc == None:
                    await ctx.send("```No me pude conectar al chat de voz...```") #```Could not connect to the voice channel```
                    return
            else:
    ...
    async def play(self, ctx, *, search):
        try:
            ...
            if not voice_channel:
                await ctx.send("No estas en un canal de voz...") # Youre not in a voice channel
    ...
    async def skip(self, ctx):
        if ctx.voice_client and ctx.voice_client.is_playing():
            ...
            await ctx.send("Skipeado") #Skipped
            await self.play_next(ctx)
    ...
    async def queue(self, ctx):
        try: # string indices must be integers, not 'str'
            ...
            if retval != "":
                ...
                await ctx.send("```No hay musica en la cola```") # No music in queue
        except Exception as error:
            print(error)
    ...
    async def erases(self, ctx):
        try:
            if self.vc != None and self.is_playing:
                self.vc.stop()
            self.music_queue = []
            await ctx.send("```Cola de la musica vaciada```") # Music queue cleared
        except Exception as error:
            print(error)
    ...
    async def re(self, ctx):
        try:
            ...
            await ctx.send("```La ultima cancion se ha removido```") # last song removed
        except Exception as error:
            print(error)
